...
def change_to_string(min):  # 분으로 된 것을 HH:MM string으로 바꿔주는 함수
    hour = str(min // 60)  # 시간
    minute = str(min % 60)  # 분
...
def solution(n, t, m, timetable):
    ...
    if m > len(timetable_min):
        answer = change_to_string(last_bus_time)  # 마지막 버스 타면 됨
    ...
    ## 마지막 버스를 타려고 기다리는 인원 계산
        ...
        if m > wait_last_num:  # 제치지 않고도 마지막 버스를 탈 수 있음
            answer = change_to_string(last_bus_time)

        else:  # m == wait_last_num, # 마지막 버스를 타기엔 탈 수 있는 마지막 사람을 제쳐야함
            answer = change_to_string(timetable_last_bus[-1] - 1)  # 마지막 사람보다 1분 빨라야함
